const createBoard = (rows, columns) => {
  return Array(rows)
    .fill(0)
    .map((_, row) =>
      Array(columns)
        .fill(0)
        .map((_, column) => ({
          row,
          column,
          opened: false,
          flagged: false,
          mined: false,
          nearMines: 0,
        }))
    );
};

const spreadMines = (board, minesAmount) => {
  const rows = board.length;
  const columns = board[0].length;
  let minesPlanted = 0;
  while (minesPlanted < minesAmount) {
    const rowSel = parseInt(Math.random() * rows, 10);
    const colSel = parseInt(Math.random() * columns, 10);

    if (!board[rowSel][colSel].mined) {
      board[rowSel][colSel].mined = true;
      minesPlanted++;
    }
  }
};

const cloneBoard = board =>
  board.map(rows => rows.map(field => ({ ...field })));

const getNeighbors = (board, row, column) => {
  const neighbors = [];
  const rows = [row - 1, row, row + 1];
  const columns = [column - 1, column, column + 1];

  rows.forEach(r => {
    columns.forEach(c => {
      const isDifferent = r !== row || c !== column;
      const isValidRow = r >= 0 && r < board.length;
      const isValidColumn = c >= 0 && c < board[0].length;

      if (isDifferent && isValidRow && isValidColumn) {
        neighbors.push(board[r][c]);
      }
    });
  });

  return neighbors;
};

const safeNeighborhood = (board, row, column) => {
  const isSafe = (result, neighbor) => result && !neighbor.mined;
  return getNeighbors(board, row, column).reduce(isSafe, true);
};

const openField = (board, row, column) => {
  const field = board[row][column];

  if (!field) return;

  if (!field.opened) {
    field.opened = true;
    if (field.mined) {
      field.exploded = true;
    } else if (safeNeighborhood(board, row, column)) {
      getNeighbors(board, row, column).forEach(f =>
        openField(board, f.row, f.column)
      );
    } else {
      const neighbors = getNeighbors(board, row, column);
      field.nearMines = neighbors.filter(n => n.mined).length;
    }
  }
};

const fields = board => board.flat();

const hasExploded = board =>
  fields(board).filter(field => field.exploded).length > 0;

const isPending = field =>
  (field.mined && !field.flagged) || (!field.mined && !field.opened);

const wonGame = board => fields(board).filter(isPending).length === 0;

const showMines = board =>
  fields(board)
    .filter(field => field.isMined)
    .forEach(field => (field.opened = true));

const createMinedBoard = (row, columns, minesAmount) => {
  const board = createBoard(row, columns);
  spreadMines(board, minesAmount);
  return board;
};

const invertFlag = (board, row, column) => {
  const field = board[row][column];
  field.flagged = !field.flagged;
};

const flagsUsed = board => fields(board).filter(f => f.flagged).length;

export {
  createMinedBoard,
  cloneBoard,
  openField,
  hasExploded,
  wonGame,
  showMines,
  invertFlag,
  flagsUsed,
};
